...
# ⚠️ QFont se usa para cambiar el tipo y tamaño de letra, aparece muy poco en las guías, pero se puede mantener si se simplifica
from PySide6.QtGui import QFont, QIcon, QPixmap  # ⚠️ QIcon y QPixmap NO se han visto en las guías
...
class FileOrganizer(QMainWindow):
    def __init__(self):
        ...
        # ⚠️ Estilos avanzados con colores y bordes redondeados (no tratados en guías aún)
        self.setStyleSheet("background-color: #2E3440; color: #D8DEE9; font-size: 14px;")

        # ⚠️ Imagen (icono centrado) con QPixmap – no se ha enseñado todavía
        self.header_label = QLabel()
        pixmap = QPixmap("search_icon.png")  # ⚠️ Se carga una imagen para mostrar arriba
        ...
        self.label.setFont(QFont("Arial", 12))  # ⚠️ Se usa QFont (puede mantenerse)

        # ✅ Campo de entrada de texto
        self.path_input = QLineEdit()
        self.path_input.setPlaceholderText("Ejemplo: C:/Usuarios/TuUsuario/Descargas")
        # ⚠️ Estilo con CSS avanzado
        self.path_input.setStyleSheet("background-color: #4C566A; color: white; border-radius: 5px; padding: 5px;")

        # ⚠️ Botón con icono – `QIcon` no se ha visto
        self.browse_button = QPushButton()
        self.browse_button.setIcon(QIcon("search_icon.png"))  # ⚠️ QIcon con imagen
        ...
        # ⚠️ Estilo avanzado no explicado aún
        ...
        layout = QVBoxLayout()
        layout.addWidget(self.header_label)  # ⚠️ Usa imagen
    ...
    def find_heavy_files(self):
        ...
        # ⚠️ Aquí se usa HTML para mostrar colores y formato – no se ha explicado aún
        ...
        self.result_display.setHtml(result_text)  # ⚠️ No se ha visto `setHtml()`
